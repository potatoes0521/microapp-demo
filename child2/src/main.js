 ...
 * @LastEditTime: 2022-06-21 14:53:58
 * @Author:
 */
...
function handleMicroData(router) {
  ...
  if (window.__MICRO_APP_ENVIRONMENT__) {
    ...
    window.microApp.addDataListener(({ event, data }) => {
      switch (event) {
        case "routerPush":
          console.log("BI收：基座 routerPush", data);
          // 当基座下发path时进行跳转
          ...
          if (data.path && data.path !== curRouterPath) {
            ...
            if (window.parentGetSys().includes("qishi-bi")) {
              router.push(data.path);
            } else {
              window.parentRouter.push(`/qishi-bi${data.path}`);
            }
          }
          break;
        case "deleteCacheTabs":
          console.log("BI收：基座 deleteCacheTabs", data);
          store.dispatch("deleteCachesTabs", data);
          break;
        default:
          console.log("BI收：基座下发数据改变");
      }
    });
    ...
    window.addEventListener("appstate-change", async function (e) {
      if (e.detail.appState === "afterhidden") {
        ...
        console.log("BI 已经重新渲染", router.currentRoute.value);
        ...
        // eg: 主站打开CC  Apage  然后打开 BI  Bpage 然后回CC 某page  如果当前来个强提醒进入BI 工单详情 这里会丢失连接上参数
        // 在跳转的时候拼接参数
