...
function handleMicroData(router) {
  ...
  if (window.__MICRO_APP_ENVIRONMENT__) {
    ...
    window.microApp.addDataListener(({ event, data }) => {
      switch (event) {
        case "routerPush":
          console.log("CAS收：基座 routerPush", data);
          // 当基座下发path时进行跳转
        ...
        case "deleteCacheTabs":
          console.log("CAS收：基座 deleteCacheTabs", data);
          store.dispatch("deleteCachesTabs", data);
          break;
        default:
          console.log("CAS收：基座下发数据改变");
      }
    });
    ...
    window.addEventListener("appstate-change", async function (e) {
      if (e.detail.appState === "afterhidden") {
        ...
        console.log("CAS 已经重新渲染", router.currentRoute.value);
        ...
        // eg: 主站打开CC  Apage  然后打开 CAS  Bpage 然后回CC 某page  如果当前来个强提醒进入CAS 工单详情 这里会丢失连接上参数
        // 在跳转的时候拼接参数
